...
| Enforcing spend limits | **The chain**, for a vault; **this software**, otherwise | The vault's cap lives in the smart account's `__check_auth` and holds even if every process we run disappears. The per-host policy file is enforced in our code — a promise our code keeps, which is weaker, and is why the vault exists. |
